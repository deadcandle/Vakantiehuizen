        ...
        if ($result -> num_rows > 0) {
            while ($row = $result -> fetch_assoc()) {
                if (in_array($row["id"], $houseIDS)) {
                    echo "<script>var miniImg = document.createElement('img'); miniImg.className = 'miniImg'; miniImg.src = 'images/".$row["afbeelding"]."'; document.getElementById('".$row["id"]."').appendChild(miniImg)</script>";
                } else {
